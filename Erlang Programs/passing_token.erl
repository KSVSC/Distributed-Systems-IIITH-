-module('2018121005_1').
-export([main/1, start/4, init_process/3]).
...
create_process(N, M, NextOnTheRing, File2) ->
  ID = N-1,
  Pid = spawn(?MODULE, init_process, [NextOnTheRing, ID, File2]),
...
receive_from_last_node(Msg, ID, File2) ->
  receive
      { Msg, From } ->
      % io:format("Process ~w received token ~w from process ~w\n", [0, Msg, ID]),
      io:format(File2, "Process ~w received token ~w from process ~w", [0, Msg, ID]),
      ok
  end.